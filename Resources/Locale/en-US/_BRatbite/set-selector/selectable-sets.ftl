# RSO hardsuit Selector

selectable-set-rso-hardsuit-name = 'goliath' experimental combat hardsuit
selectable-set-rso-hardsuit-desc =
    A slow, heavily armored combat hardsuit designed as a walking tank.
    Extremely bulky but highly durable, it includes an automated chemical injector
    that stabilizes the wearer and aids recovery from critical injury.

selectable-set-rso-voidsuit-name = 'magistrate' light combat voidsuit
selectable-set-rso-voidsuit-desc =
     A lightweight combat voidsuit focused on speed and agility over protection.
     Its reduced armor allows rapid movement, while the helmet replaces standard lighting
     with integrated night vision for superior performance in darkness.

# RSO Weapon Selector

selectable-set-rso-weaponkit-smartgun-name = Abielle Smart-SMG Set
selectable-set-rso-weaponkit-smartgun-desc =
    you will never miss with this weapon.
    Include : Abielle Smart-SMG and 2 smart magazine.

# ERT Weapon Selector

selectable-set-ert-rapier-kit-name = Phoron Rapier Set
selectable-set-ert-rapier-kit-desc =
    When gun fail or when you want to aurafarm.
    Include : Phoron rapier, greatshield and dragonfu book.

selectable-set-ert-null-rods-kit-name = Null Rods Set
selectable-set-ert-null-rods-kit-desc =
    The classic way to fight evil. Just like god intended.
    Include : Nullrod and dragonfu book.

selectable-set-ert-holy-rapier-kit-desc =
    Alternative method than nullrod provided by Nanotransen.
    Include : Holy phoron rapier, greatshield, and dragonfu.
