...
using Content.Ratbite.Shared.CCVar;
...
public sealed class HeightAdjustSystem : EntitySystem
{
    ...
    public bool SetScale(EntityUid uid, Vector2 scale)
    {
        ...
        if (_config.GetCVar(RatbiteCVars.HeightAdjustModifiesZoom) && TryComp<ContentEyeComponent>(uid, out var eye))
            _eye.SetMaxZoom(uid, eye.MaxZoom * avg);
        else
            succeeded = false;

        if (_config.GetCVar(RatbiteCVars.HeightAdjustModifiesHitbox) && TryComp<FixturesComponent>(uid, out var fixtures))
