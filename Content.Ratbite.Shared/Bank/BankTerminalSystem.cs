...
public sealed partial class BankTerminalSystem : EntitySystem
{
    ...
    private void OnOpen(Entity<BankTerminalComponent> ent, ref BeforeActivatableUIOpenEvent args)
    {
        ...
        _ui.SetUiState(ent.Owner, BankTerminalUiKey.Key, new BankTerminalInterfaceState(ent.Comp.LinkedAccount, ent.Comp.LinkedPassword, GetNetEntity(ent.Comp.LinkedBank)));
    }
    ...
    private void OnInsertMoney(Entity<BankTerminalComponent> ent, ref InteractUsingEvent args)
    {
        ...
        {
            _popup.PopupPredicted($"{args.Used} is not a valid currency.", args.User, args.User, PopupType.Medium);
            return;
        }
