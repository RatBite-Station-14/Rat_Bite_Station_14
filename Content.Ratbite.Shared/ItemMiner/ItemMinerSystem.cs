    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var miner, out var xform))
        {
                ...
                || miner.NeedsAnchored && !xform.Anchored)
            {
                ...
                PredictedQueueDel(miner.AudioUid);
                continue;
            }
            ...
            if (miner.NextAt < _timing.CurTime)
            {
                ...
                {
                    var slotEnt = PredictedSpawnAtPosition(miner.Proto, xform.Coordinates);
                ...
                EntityUid minedUid = slotItem ?? PredictedSpawnAtPosition(miner.Proto, xform.Coordinates);
                if (slotItem == null)
                    spawned++;
                ...
                else if (slot == null)
                {
                    for (var i = 0; i < miner.Amount - spawned; i++)
                        PredictedSpawnAtPosition(miner.Proto, xform.Coordinates);
